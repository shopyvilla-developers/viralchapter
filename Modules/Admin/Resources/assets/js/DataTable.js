...
CloudCMS.dataTable = { routes: {}, selected: {} };
...
export default class {
    constructor(selector, options, callback) {
        ...
        if (CloudCMS.dataTable.selected[selector] === undefined) {
            CloudCMS.dataTable.selected[selector] = [];
        }
    ...
    appendToSelected(id) {
        ...
        if (! CloudCMS.dataTable.selected[this.selector].includes(id)) {
            CloudCMS.dataTable.selected[this.selector].push(id);
        }
    }
    ...
    removeFromSelected(id) {
        ...
        CloudCMS.dataTable.selected[this.selector].remove(id);
    }
    ...
    route(name, params) {
        let router = CloudCMS.dataTable.routes[this.selector][name];
    ...
    hasRoute(name) {
        return CloudCMS.dataTable.routes[this.selector][name] !== undefined;
    }

    static setRoutes(selector, routes) {
        CloudCMS.dataTable.routes[selector] = routes;
    }

    static setSelectedIds(selector, selected) {
        CloudCMS.dataTable.selected[selector] = selected;
    }

    static getSelectedIds(selector) {
        return CloudCMS.dataTable.selected[selector];
    }
