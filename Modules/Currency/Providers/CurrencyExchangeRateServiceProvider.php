...
class CurrencyExchangeRateServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $this->app->singleton(CurrencyRateExchanger::class, function () {
            ...
            $options = config("CloudCMS.module.currency.config.services.{$service}");
    ...
    {
        config([
            'CloudCMS.module.currency.config.services.fixer.access_key' => setting('fixer_access_key'),
            'CloudCMS.module.currency.config.services.forge.api_key' => setting('forge_api_key'),
            'CloudCMS.module.currency.config.services.currency_data_feed.api_key' => setting('currency_data_feed_api_key'),
        ]);
    }
}
