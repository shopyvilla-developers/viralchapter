...
use Modules\Core\Foundation\Asset\Manager\CloudCMSAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\CloudCMSAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, CloudCMSAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new CloudCMSAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (! is_null($assets = config("CloudCMS.theme.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("CloudCMS.module.{$module->getLowerName()}.assets");
